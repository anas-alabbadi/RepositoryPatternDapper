//// This classes has been downloaded from https://github.com/StackExchange/Dapper/tree/master/Dapper.Contrib
using System;
...
public partial interface ISqlAdapter
{
    ...
    Task<int> InsertAsync(IDbConnection connection, IDbTransaction transaction, int? commandTimeout, string tableName, string columnList, string parameterList, IEnumerable<PropertyInfo> keyProperties, object entityToInsert,bool isRowEffected);
}
...
public partial class SqlServerAdapter
{
    ...
    /// <param name="entityToInsert">The entity to insert.</param>
    /// <returns>The Id of the row created.</returns>
    public async Task<int> InsertAsync(IDbConnection connection, IDbTransaction transaction, int? commandTimeout, string tableName, string columnList, string parameterList, IEnumerable<PropertyInfo> keyProperties, object entityToInsert,bool isRowEffected=false)
    {
        string _returnType = isRowEffected ? "SELECT @@ROWCOUNT id" : "SELECT SCOPE_IDENTITY() id";
        var cmd = $"INSERT INTO {tableName} ({columnList}) values ({parameterList}); {_returnType}";
...
public partial class MySqlAdapter
{
    ...
    public async Task<int> InsertAsync(IDbConnection connection, IDbTransaction transaction, int? commandTimeout, string tableName,
        string columnList, string parameterList, IEnumerable<PropertyInfo> keyProperties, object entityToInsert, bool isRowEffected = false)
    {
        var cmd = $"INSERT INTO {tableName} ({columnList}) VALUES ({parameterList})";
        await connection.ExecuteAsync(cmd, entityToInsert, transaction, commandTimeout).ConfigureAwait(false);
